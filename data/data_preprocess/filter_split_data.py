...
train_list = set(fileToList("./AIOZ_Dataset/train_split_sequence_names.txt"))
test_list = set(fileToList("./AIOZ_Dataset/val_split_sequence_names.txt"))
...
def split_data(dataset_path):
    ...
    for split_list, split_name in zip([train_list, test_list], ["train", "test"]):
        ...
        for sequence in split_list:
            motion = f"{dataset_path}/motions_smpl/{sequence}.pkl"
            wav = f"{dataset_path}/musics438/{sequence}.wav"
            assert os.path.isfile(motion)
            assert os.path.isfile(wav)
            motion_data = pickle.load(open(motion, "rb"))
            trans = motion_data["root_trans"]
            pose = motion_data["smpl_poses"]
            betas = motion_data["smpl_betas"]
            meta = motion_data["meta"]
            out_data = {"pos": trans, "q": pose, "betas": betas,"meta":meta}
