...
# Update root name according to FBX node names.
root_name = "m_avg_Pelvis" # origin
ignore_list = []
...
class FbxReadWrite(object):
    ...
    def addAnimation(self, pkl_filename: str, smpl_params: Dict, verbose: bool = False):
        ...
        for idx, name in enumerate(names):
            node = lRootNode.FindChild(name) # 没有办法 findChild
            rotvec = smpl_poses[:, idx * 3 : idx * 3 + 3]

            # if name == "m_avg_Pelvis":
            # if name == "mixamorig1:Hips": # mixamo1
            # if name == "mixamorig:Hips": # mixamo
            if name == root_name:
